...
void makeBranchArray(const Octree<Cell>::Branch* currentBranch, std::vector<GPUBranch*>& branches, std::vector<GPULeaf*>& leaves,
...
void packOctree(const Octree<Cell>& octree, std::vector<unsigned int>& branchGPU, std::vector<unsigned int>& leavesGPU, int& branchCount)
{
	std::vector<GPUBranch*> branches;
	std::vector<GPULeaf*> leaves;
	int leafId = 1;
	
	makeBranchArray(static_cast<const Octree<Cell>::Branch*>(octree.root()), branches, leaves, branchCount, leafId);
	...
	for (int i = 0; i < leaves.size(); i++)
	{
		...
		int coeffCount = leaves[i]->value->poly.coeffs.size();
		for (int k = 0; k < coeffCount; k += 4)
		{
			glm::vec4 coeffVec4 = glm::vec4(0.0f);
			if (4 * k + 0 < coeffCount) coeffVec4.x = leaves[i]->value->poly.coeffs[4 * k + 0];
			if (4 * k + 1 < coeffCount) coeffVec4.y = leaves[i]->value->poly.coeffs[4 * k + 1];
			if (4 * k + 2 < coeffCount) coeffVec4.z = leaves[i]->value->poly.coeffs[4 * k + 2];
			if (4 * k + 3 < coeffCount) coeffVec4.w = leaves[i]->value->poly.coeffs[4 * k + 3];
